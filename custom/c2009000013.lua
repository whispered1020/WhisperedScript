...
function s.initial_effect(c)
	--Add 1 "Abyssal Dread" monster from deck
	...
	c:RegisterEffect(e1)
	--Add 1 "Abyssal Dread" Spell/Trap from deck
...
--Add "Abyssal Dread" monster to hand
...
--Add 1 "Abyssal Dread" Spell/Trap from deck
function s.thfilter2(c,ft)
	return c:IsSetCard(0xf18) and c:IsSpellTrap() and c:IsAbleToHand()
end
function s.thtg2(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chk==0 then return Duel.IsExistingMatchingCard(s.thfilter2,tp,LOCATION_DECK,0,1,nil) end
	Duel.SetOperationInfo(0,CATEGORY_TOHAND,1,1,tp,LOCATION_DECK)
end
